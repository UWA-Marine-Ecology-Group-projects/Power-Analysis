...
h <- "auv_detailed.percent.cover_20200914.csv"
...
str(hab)
hab$campaignid <- as.factor(hab$campaignid)
...
#write.csv(hab, paste(d.dir, "GB_hab_AUV_grid_20200914.csv", sep='/'))

####    Multivariate    ----

# Load hab data ----

f <- read.csv(paste(d.dir, "GB_hab_AUV_grid_20200914.csv", sep='/'))


# Euclidian ----
names(f)
str(f)
f$campaignid <- as.factor(f$campaignid)
head(f)
...
fb.plot<-cbind(f[-c(1:5)], nmdsb1, nmdsb2, cluster= f[,2])
fb.plot
