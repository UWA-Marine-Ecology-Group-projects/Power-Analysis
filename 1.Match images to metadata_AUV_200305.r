...
auv <- read.csv(paste(w.dir, "Data", "raw", "auv_broad.percent.cover.csv", sep='/'))
...
write.csv(auvcatdf, paste(w.dir, "Data", "tidy", "Auv_zoning.csv", sep='/'))



### So far up to here, and I've been organizing the data for Power analysis in that script ###


### split the data by zone name
auv_list <- split(bruvscatdf, bruvscatdf$ZoneName)
str(bruvs_list)
